...
import { adminGetRetiros, adminMarcarEntregado, adminRevertirEntrega, type RetiroAdmin } from "../../lib/retiros";

type Tab = "estadisticas" | "usuarios" | "farmacias" | "retiros";
...
export function AdminPanel() {
  ...
  const [busquedaRetiros, setBusquedaRetiros] = useState("");
  const [fechaDesde, setFechaDesde] = useState("");
  const [fechaHasta, setFechaHasta] = useState("");
  const [filtroEstado, setFiltroEstado] = useState<"todos" | "planificado" | "entregado">("todos");
  ...
  async function loadRetiros() {
    ...
    try {
      setRetiros(await adminGetRetiros(token));
    } catch (e: unknown) {
      if (e instanceof Error && e.message === "UNAUTHORIZED") { handleUnauthorized(); return; }
      setMensaje("Error al cargar retiros");
    }
  }
  ...
  async function handleEntregar(id: number) {
    ...
    setCargando(true);
    try {
      await adminMarcarEntregado(token, id);
  ...
  async function handleRevertir(id: number) {
    if (!confirm("¿Revertir este retiro a pendiente? Se restaurará el stock de la farmacia.")) return;
    const token = getToken();
    if (!token) { handleUnauthorized(); return; }
    setCargando(true);
    try {
      await adminRevertirEntrega(token, id);
      setMensaje("Retiro revertido a pendiente");
      loadRetiros();
      setTimeout(() => setMensaje(""), 3000);
    } catch (e: unknown) {
      if (e instanceof Error && e.message === "UNAUTHORIZED") { handleUnauthorized(); return; }
  ...
  const retirosFiltrados = retiros.filter(r => {
    const q = busquedaRetiros.toLowerCase().trim();
    if (q
      && !`${r.usuario_nombre} ${r.usuario_apellido}`.toLowerCase().includes(q)
      && !r.cedula.includes(q)
      && !r.farmacia_nombre.toLowerCase().includes(q)) return false;
    if (fechaDesde && r.fecha_retiro < fechaDesde) return false;
    if (fechaHasta && r.fecha_retiro > fechaHasta) return false;
      ...
      <main className="p-4 max-w-5xl mx-auto">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-[#f0f0f8]">
                    {cargandoUsuarios ? (
                      <tr>
                        <td colSpan={5} className="px-5 py-14 text-center text-gray-300 text-sm">Cargando...</td>
                      </tr>
                    ) : errorUsuarios ? (
                      <tr>
                        <td colSpan={5} className="px-5 py-14 text-center text-red-400 text-sm">{errorUsuarios}</td>
                      </tr>
                    ) : usuarios.length === 0 ? (
                      <tr>
                        <td colSpan={5} className="px-5 py-14 text-center">
                          ...
                          <p className="text-gray-300 text-sm">No se encontraron usuarios</p>
                        </td>
                      </tr>
                    ) : (
                      usuarios.map(u => (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button
                      onClick={() => handleEliminarFarmacia(f.id)}
                      title="Eliminar"
        ...
        {tab === "retiros" && (
          <div className="space-y-2">
            <style>{`
              .scroll-retiros::-webkit-scrollbar { width: 4px; }
              .scroll-retiros::-webkit-scrollbar-track { background: transparent; }
              .scroll-retiros::-webkit-scrollbar-thumb { background: rgba(109,40,217,0.15); border-radius: 9999px; }
              .scroll-retiros::-webkit-scrollbar-thumb:hover { background: rgba(109,40,217,0.30); }
            `}</style>

            {/* Barra de filtros */}
            <div className="bg-white rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.06)] px-3 py-2.5 flex flex-wrap items-center gap-2">
              <div className="relative flex-1 min-w-[160px]">
                <svg className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-300 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
                </svg>
                <input
                  type="text"
                  placeholder="Nombre, cédula o farmacia..."
                  value={busquedaRetiros}
                  onChange={e => setBusquedaRetiros(e.target.value)}
                  className="w-full bg-[#f9f9fd] rounded-lg pl-8 pr-3 py-1.5 text-xs border-0 focus:outline-none focus:ring-2 focus:ring-violet-400/20 transition-all duration-200 placeholder:text-gray-300"
                />
              </div>
              <div className="flex items-center gap-1.5 shrink-0">
                <input
                  ...
                  title="Desde"
                  className="bg-[#f9f9fd] rounded-lg px-2.5 py-1.5 text-xs text-gray-500 border-0 focus:outline-none focus:ring-2 focus:ring-violet-400/20 transition-all duration-200 cursor-pointer"
                />
              ...
              <select
                value={filtroEstado}
                onChange={e => setFiltroEstado(e.target.value as "todos" | "planificado" | "entregado")}
                className="bg-[#f9f9fd] rounded-lg px-2.5 py-1.5 text-xs text-gray-500 border-0 focus:outline-none focus:ring-2 focus:ring-violet-400/20 transition-all duration-200 cursor-pointer shrink-0"
              >
                <option value="todos">Todos</option>
                <option value="planificado">Pendiente</option>
                <option value="entregado">Entregado</option>
              </select>
              {(busquedaRetiros || fechaDesde || fechaHasta || filtroEstado !== "todos") && (
                <button
                  onClick={() => { setBusquedaRetiros(""); setFechaDesde(""); setFechaHasta(""); setFiltroEstado("todos"); }}
                  title="Limpiar filtros"
                  className="shrink-0 p-1.5 rounded-lg bg-[#f9f9fd] hover:bg-red-50 text-gray-300 hover:text-red-400 transition-all duration-200 cursor-pointer"
                >
              ...
              <span className="text-xs text-gray-300 shrink-0 ml-auto tabular-nums">
                {retirosFiltrados.length}{retiros.length !== retirosFiltrados.length ? ` / ${retiros.length}` : ""} retiros
              </span>
            </div>
            ...
            <div className="relative">
              <div
                className="scroll-retiros overflow-y-auto space-y-1.5 pr-1"
                style={{ maxHeight: "calc(100vh - 230px)" }}
              >
                {retiros.length === 0 && (
                  <div className="bg-white rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.06)] p-10 text-center text-gray-300 text-sm">
                    No hay retiros registrados
                  </div>
                )}
                {retiros.length > 0 && retirosFiltrados.length === 0 && (
                  <div className="bg-white rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.06)] p-10 text-center text-gray-300 text-sm">
                ...
                {retirosFiltrados.map((r, i) => (
                  <div
                    key={r.id}
                    className="bg-white rounded-xl shadow-[0_1px_4px_rgba(0,0,0,0.05)] px-4 py-3 animate-fade-in-up hover:shadow-[0_2px_8px_rgba(0,0,0,0.08)] transition-shadow duration-200"
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {r.estado === "planificado" && (
                            <span className="shrink-0 text-xs px-2 py-0.5 rounded-full bg-violet-50 text-violet-600 font-semibold">
                              Pendiente
                            </span>
                          )}
                        </div>
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-1 text-xs text-gray-400">
                          <span className="font-mono">{r.cedula} · {r.telefono}</span>
                      ...
                      <div className="shrink-0">
                        {r.estado === "planificado" ? (
                          <button
                            ...
                            disabled={cargando}
                            className="flex items-center gap-1.5 bg-emerald-500 hover:bg-emerald-600 disabled:bg-gray-100 disabled:text-gray-300 text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition-all duration-200 active:scale-[0.97] cursor-pointer shadow-[0_2px_6px_rgba(16,185,129,0.3)]"
                          >
                          ...
                          <div className="flex items-center gap-1.5">
                            <span className="flex items-center gap-1 text-xs bg-[#f0f0f8] text-gray-500 font-semibold px-2.5 py-1.5 rounded-lg">
                            ...
                            <button
                              ...
                              className="p-1.5 rounded-lg bg-[#f0f0f8] hover:bg-amber-50 text-gray-400 hover:text-amber-500 disabled:opacity-40 transition-all duration-200 cursor-pointer"
                            >
                              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
