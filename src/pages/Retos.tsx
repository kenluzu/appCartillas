...
export function Retos() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setError((data.error as string) ?? "Error al registrar");
        return;
      }

      const nuevaCartilla = data.cartilla as { id: number; puntos: number; estado: "activa" | "completa" | "cerrada"; fecha_inicio: string };
            ...
            <div className="px-5 py-5">
              {exito ? (
                <div className="text-center py-4">
                  ...
                  <button
                    onClick={cerrarModal}
                    className="mt-5 w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-2xl transition-colors cursor-pointer text-sm"
                  >
                    Continuar
                  </button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  {cfg.labelExtra && (
                    <div>
                      ...
                      <input
                        ...
                        onChange={e => setForm(p => ({ ...p, campo_extra: e.target.value }))}
                        required
                        className="w-full bg-gray-50 rounded-xl px-4 py-2.5 text-sm border border-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-300 transition-all"
                      />
                    </div>
                  )}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        required
                        placeholder={`${cfg.montoMin}.00`}
                        className="w-full bg-gray-50 rounded-xl pl-8 pr-4 py-2.5 text-sm border border-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-300 transition-all"
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2 pt-1">
                    <button
                      type="button"
                      onClick={cerrarModal}
                      className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-600 font-bold py-3 rounded-2xl transition-colors cursor-pointer text-sm"
                    >
                      Cancelar
                    </button>
                    <button
                      type="submit"
                      disabled={enviando}
                      className={`flex-1 ${cfg.colorBtn} disabled:bg-gray-100 disabled:text-gray-400 text-white font-bold py-3 rounded-2xl transition-colors cursor-pointer text-sm shadow-sm`}
                    >
